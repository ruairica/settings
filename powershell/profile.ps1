...
function Do-RunBicepFormatRecursive {
    $currentFolder = Get-Location
    $bicepFiles = Get-ChildItem -Path $currentFolder.ProviderPath -Recurse -Filter *.bicep

    foreach ($file in $bicepFiles) {
        az bicep format --file $file.FullName
    }
}

# open the .sln or .slnx in the current folder in Visual studio
function Do-OpenVisualStudio {
    $currentFolder = Get-Location
    $sln = Get-ChildItem -Path $currentFolder.ProviderPath -Filter *.sln* -File | Where-Object { $_.Extension -in '.sln', '.slnx' } | Select-Object -First 1

    if ($sln) {
        Start-Process $sln.FullName
    }
    else {
        Write-Warning "No solution file (.sln or .slnx) found in the current directory."
    }
}
...
function Do-GoToAocFolder {
    Set-Location 'C:\Source\personal\AdventOfCode\AdventOfCode\CSharp\Aoc'
    Do-OpenVisualStudio
...
#helper function to quit lf (list files) in the current directory
# lf configuration file
# put the below comment into ~\AppData\Local\lf\lfrc
# Quit and change to selected directory with 'q' key
# Opens/enters the selected item, then quits
# This works with the PowerShell lf function that uses -last-dir-path
# map q push :open<enter>:quit<enter>
function lf {
...
$global:__LastOutput = $null
function Out-Default {
    $collected = @($input)
    $global:__LastOutput = $collected
    $collected | Microsoft.PowerShell.Core\Out-Default
}
...
Set-Alias cprev Copy-LastOutput

# --- Deferred: heavy items that run once on first prompt ---
# This defers module imports, PSReadLine config, and oh-my-posh init
# so the shell becomes interactive faster.

$global:_DeferredLoadDone = $false

function global:_DeferredLoad {
    ...
    Set-PSReadLineKeyHandler -Key Ctrl+Shift+t `
        -BriefDescription BuildCurrentDirectory `
        -LongDescription "Build the current directory with tests" `
        -ScriptBlock {
        [Microsoft.PowerShell.PSConsoleReadLine]::RevertLine()
    ...
    # oh-my-posh (the heaviest item at ~253ms)
    oh-my-posh init pwsh --config 'C:\Users\ruairi.caldwell\AppData\Local\oh-my-posh\themes\ruairi2.omp.json' | Invoke-Expression
}

# Hook deferred load into the first prompt.
# After _DeferredLoad runs, oh-my-posh will have replaced $function:prompt
# with its own, so we just call the current prompt function (which is now oh-my-posh's).
$function:prompt = {
    _DeferredLoad
    & $function:prompt
}
