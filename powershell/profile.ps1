...
Import-Module z
oh-my-posh init pwsh --config 'C:\Users\Ruairi.Caldwell\AppData\Local\Programs\oh-my-posh\themes\ruairi2.omp.json' | Invoke-Expression
...
# open the .sln in the current folder in Visual studio
function Do-OpenVisualStudio {
    $currentFolder = Get-Location
    $sln = Get-ChildItem -Path $currentFolder.ProviderPath -Filter *.sln

    Start-Process $sln.Name
}
